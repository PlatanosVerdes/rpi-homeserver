...
| `since Sep 12 00:41` | `.StartsAt`, in the Pi's timezone. The Grafana service is given `TZ` for this: without it Go formats in UTC and every alert reads two hours early. The date is there because `repeat_interval` re-sends a warning that is still firing days later, and a bare hour on that message reads as today |
